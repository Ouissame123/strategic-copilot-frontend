...
import type { CitationType, ParsedCitation } from "@/api/manager-copilot.types";
import { citationEntityPath } from "@/lib/helper-chat-citations";
...
export function CitationChip({ citation, projectId }: { citation: ParsedCitation; projectId?: string }) {
    const navigate = useNavigate();
    const cfg = CONFIG[citation.type];
    const Icon = cfg.Icon;

    const handleClick = () => {
        const path = citationEntityPath(citation.type, citation.id);
        if (path) {
            navigate(path);
            return;
        }
        if (citation.type === "alert" && projectId) {
            navigate(managerProjectMissionControlPath(projectId, "overview"));
            return;
        }
        if (citation.type === "decision" && projectId) {
            navigate(managerProjectMissionControlPath(projectId, "overview"));
        }
    };
