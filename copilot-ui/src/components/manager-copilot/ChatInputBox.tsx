...
export function ChatInputBox({ onSend, isLoading, placeholder, disabled }: Props) {
        ...
        <div className="border-t border-secondary bg-primary p-3">
            <div className="flex items-end gap-2">
                <textarea
                    ...
                    placeholder={placeholder ?? "Pose une question…"}
                    disabled={isLoading || disabled}
                    rows={1}
                    maxLength={4000}
                    aria-label="Message pour le Helper IA"
                    className={cx(
            ...
            <p className="mt-1.5 text-[10px] text-fg-quaternary">
                Enter = envoyer · Shift+Enter = nouvelle ligne · max 4000 chars
            </p>
        </div>
    );
}
