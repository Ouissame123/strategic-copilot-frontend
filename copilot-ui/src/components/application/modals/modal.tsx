...
export const ModalOverlay = (props: AriaModalOverlayProps) => {
    return (
        <AriaModalOverlay
            ...
            className={(state) =>
                cx(
                    /** Toujours centrer la modale dans le viewport (evite le coin haut-gauche). */
...
export const Modal = (props: AriaModalOverlayProps) => (
    <AriaModal
        ...
        className={(state) =>
            cx(
                /** Conteneur flex : le panneau (Dialog) reste centré même si le contenu est large. */
                "flex max-h-full min-h-0 w-full max-w-full min-w-0 flex-col items-center justify-center align-middle outline-hidden max-sm:overflow-y-auto max-sm:rounded-xl",
...
export const Dialog = (props: AriaDialogProps) => (
    <AriaDialog
        {...props}
        className={cx("flex w-full max-w-full min-w-0 flex-col items-center justify-center outline-hidden", props.className)}
    />
);
