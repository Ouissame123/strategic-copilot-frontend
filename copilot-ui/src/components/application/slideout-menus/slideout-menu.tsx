...
export const Modal = (props: ModalProps) => (
    <AriaModal
        ...
        className={(state) =>
            cx(
                "inset-y-0 right-0 h-full w-full max-w-[420px] overflow-hidden rounded-l-2xl shadow-2xl ring-1 ring-secondary/80 transition",
...
export const Dialog = (props: DialogProps) => (
    <AriaDialog
        ...
        {...props}
        className={cx(
            "relative flex size-full flex-col items-start gap-0 overflow-y-auto bg-primary outline-hidden",
            props.className,
        )}
    />
);
...
const Content = ({ role = "main", ...props }: ComponentPropsWithRef<"div">) => {
    return <div role={role} {...props} className={cx("flex size-full flex-col gap-5 overflow-y-auto overscroll-auto px-5 md:px-6", props.className)} />;
};
...
const Header = ({ className, children, onClose, ...props }: SlideoutHeaderProps) => {
    return (
        <header {...props} className={cx("relative z-1 w-full px-5 pt-5 md:px-6 md:pt-6", className)}>
            {children}
