...
export function ManagerPageLayout({ header, kpi, distribution, filters, main, sidebar }: ManagerPageLayoutProps) {
    return (
        <div className="min-h-0 bg-primary">
            {header}
            <div className="mx-auto max-w-[1280px] space-y-6 px-4 py-6 sm:px-6">
                {kpi}
                {distribution}
                {filters ? (
                    <div className="sticky top-14 z-20 -mx-4 border-b border-secondary/80 bg-primary/95 px-4 py-3 backdrop-blur-sm sm:-mx-6 sm:px-6">
                        {filters}
                    </div>
                ) : null}
