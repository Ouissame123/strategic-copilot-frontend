...
type RhChatPanelProps = {
    /** Mode compact pour le panneau flottant (sans sidebar ni panneau d'analyse). */
    embedded?: boolean;
};

export function RhChatPanel({ embedded = false }: RhChatPanelProps) {
    const { user } = useAuth();
    ...
    if (embedded) {
        return <div className="flex h-full min-h-0 flex-col overflow-hidden">{mainPanel}</div>;
    }

    return (
        <div
            className={cx(
                "flex h-[calc(100dvh-7.5rem)] min-h-[520px] flex-col gap-2 bg-slate-50 p-1 lg:flex-row lg:gap-3 lg:p-2 dark:bg-slate-950",
            )}
        >
            <RhChatSidebar
