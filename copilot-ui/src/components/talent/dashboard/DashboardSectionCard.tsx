...
export function DashboardSectionCard({
    ...
    return (
        <section
            className={cx(
                TALENT_SURFACE,
                TALENT_SURFACE_ACCENT[accent],
                compact ? "p-2.5" : "p-3",
                className,
            )}
        >
            <header className={cx("flex items-center justify-between gap-2", compact ? "mb-1.5" : "mb-2")}>
                <div className="flex min-w-0 items-center gap-2">
                    {accent === "ai" ? (
                        <span className="flex size-6 shrink-0 items-center justify-center rounded-md bg-violet-500/10 text-violet-600 dark:text-violet-300">
