...
function statusTone(status?: string): string {
    ...
    if (s.includes("start") || s.includes("demarr")) return "bg-primary-100 text-primary-700";
...
export function TalentProjectCard({ name, subtitle, status, progressLabel, progressValue, dueLabel }: TalentProjectCardProps) {
    ...
    return (
        <article className="rounded-xl border border-black/5 bg-white p-[18px] shadow-[0_1px_3px_rgba(0,0,0,0.06)] transition hover:-translate-y-0.5 hover:border-primary-600/45">
            ...
            <div className="mt-3 h-1 overflow-hidden rounded-full bg-[#f7f6f3]">
                <div className="h-full rounded-full bg-primary-600" style={{ width: `${value ?? 0}%` }} />
            </div>
