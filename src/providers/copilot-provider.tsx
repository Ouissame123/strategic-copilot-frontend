...
import { copilotDecisionsUrl, copilotInsightsUrl, isCopilotDevFallbackEnabled } from "@/copilot/api/copilot-api.config";
import { fetchCopilotInsights, getCopilotFetchErrorMessage, submitCopilotDecision } from "@/copilot/api/copilot-api.service";
import { runCopilotDevFallbackInsights } from "@/copilot/copilot-fallback";
        ...
        (suggestionId: string) => {
            ...
            void (async () => {
                try {
                    ...
                    } else if (isCopilotDevFallbackEnabled()) {
                        const raw = await runCopilotDevFallbackInsights(suggestionId, pageContext);
                        finishBatch(sortInsights(raw), suggestionId, { nextWhatIf: null });
                    } else {
