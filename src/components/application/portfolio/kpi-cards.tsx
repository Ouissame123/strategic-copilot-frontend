...
const KpiCard = ({ title, value, description, icon: Icon, statusColor, statusLabel }: KpiCardProps) => {
    return (
        <article className="rounded-xl border border-secondary bg-primary p-5 shadow-xs md:p-6">
...
export const KpiCards = ({ summary }: KpiCardsProps) => {
    ...
    return (
        <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-4">
            {cards.map((card) => (
