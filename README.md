...
### Connexion admin (Strategic Copilot)

Pour vous connecter en tant qu’**admin** sans savoir quel compte a été créé en premier :

**Option 1 – Fichier `.env`**  
1. Copiez `.env.example` en `.env`.  
2. Ajoutez votre e-mail : `VITE_ADMIN_EMAIL=votre@email.com`.  
3. Redémarrez le serveur (`npm run dev`).  
4. Connectez-vous avec cet e-mail et son mot de passe : vous serez admin.

**Option 2 – Console du navigateur**  
1. Ouvrez l’app (ex. http://localhost:5173).  
2. Ouvrez les outils de développement (F12) → onglet **Console**.  
3. Collez :  
   `localStorage.setItem('strategic-copilot-admin-email', 'votre@email.com')`  
   (remplacez par votre e-mail).  
4. Actualisez la page, puis connectez-vous avec cet e-mail : vous serez admin.
